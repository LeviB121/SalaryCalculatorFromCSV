...
public class SalaryCalculatorFromCSV {
    public static void main(String args[]) {
        // Arguments from the command line: csv file (full path), and calculation type (a or b)
        ...
        while (!calcTypeArg.equalsIgnoreCase("a") && !calcTypeArg.equalsIgnoreCase("b")) {
            Scanner input = new Scanner(System.in);
            System.out.println("Incorrect selection for calculation type argument! \nPlease type 'a' for total salaries, or 'b' for salaries by group:");
            calcTypeArg = input.next();
        }
        if (calcTypeArg.equalsIgnoreCase("a")) {
            e2.displayAllEmployeesAndSalaries(employees);
        } else if (calcTypeArg.equalsIgnoreCase("b")) {
